...
class NeighborTest extends AnyFlatSpec{
  ...
  private def simpleProblem3V = new {
    ...
    val directGraph: Map[FunctionConstraint2V, (Variable, Variable)] = Map((c1 -> ((var1, var2))), (c2 -> ((var2, var3))))
  ...
  it should "be able to get all the Neighbor of a Variable" in {
    ...
    val reversedGraph: Map[Variable, Map[FunctionConstraint2V, Variable]] = Map(
      (var1 -> Map(c1 -> var2)),
      (var2 -> Map(c2 -> var3)),
      (var3 -> Map())
    )
